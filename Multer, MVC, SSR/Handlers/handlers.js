	...
	async addBooks(bookObj) {
		try {
			...
			const newBook = {
				id: books.size + 1,
				...bookObj,
				authors: Array.isArray(bookObj.authors)
					? bookObj.authors
					: [bookObj.authors],
			}
...
${error}`)
	...
	async findBookByName(booksObj) {
		try {
			const books = await this.checkBooksMap()
			const { title } = booksObj
			const foundedBook = Array.from(books.values()).find(book => book.title.toLowerCase() === title.toLowerCase())
			return foundedBook ? foundedBook : false
		} catch (error) {
			console.error(`Error, can't add books in findBookByName():
${error}`)
			throw error
		}
	}
	...
	async updateBookById(id, updates) {
		try {
			...
			const updatedBook = {
				...
				file_upload: updates.file_upload
					? updates.file_upload
					: book.file_upload,
			}

			updates.file_upload
				? await this.deleteFromUploads(book['file_upload'])
				: undefined
...
${error}`
	...
	async deleteFromUploads(path) {
		try {
			await unlink(`${cwd()}/${path}`)
		} catch (error) {
			if (error.code === 'ENOENT') {
				console.error(`uploads/${filename} file doesn't exist`)
				throw error
			} else {
				console.error(
					`Error, converting map to json in deleteFromUploads():
	${error}`
				)
				throw error
			}
